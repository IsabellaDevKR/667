...
namespace plugins
{
    ...
    void draw_watermark()
    {
        ...
        {
            m_drawing->AddText((float)current_x_pos, (float)current_y_pos, current_color, current_size, FL_SHADOW, xorstr_("IsabellaDev"));

            m_drawing->AddText((float)current_x_pos, (float)current_y_pos + current_size + 2.f, current_link_color, current_size - 2.f, FL_SHADOW, xorstr_("Invite Your Friends to The Discord"));
